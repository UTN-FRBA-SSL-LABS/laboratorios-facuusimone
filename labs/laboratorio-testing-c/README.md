...
> R: Inicializa el carrito poniendo `cantidad` en 0. Es importante llamarla antes de usar el carrito porque `cantidad` es una variable local sin inicializar; si no se llama, puede tener cualquier valor de basura en memoria, lo que rompería la lógica de `carrito_agregar` y `carrito_contar`.
...
void test_total_con_cantidad(void) {
    ...
    carrito_agregar(&c, p);
    ASSERT_IGUAL(700, carrito_total(&c));  /* valor esperado completado */
}
```
...
> R: En la función `carrito_total`, la línea `total += c->items[i].precio;` — solo suma el precio unitario, sin tener en cuenta la cantidad.
...
> R: Falla. El 5to `carrito_agregar` devuelve 1 (éxito) cuando debería devolver 0, y además provoca un buffer overflow ("stack smashing detected").
...
> R: El operador incorrecto es `<=` en `if (c->cantidad <= MAX_ITEMS)`. Debería ser `<`, porque con `<=` se permite escribir en `items[MAX_ITEMS]`, una posición fuera de los límites del array (que solo tiene índices 0 a MAX_ITEMS-1).
...
> R: Sí, las líneas de la función `carrito_descuento` (líneas 29-30) aparecen con `#####`. No se ejecutaron porque `make cobertura` solo compila y corre `test_unitarios`, y ningún test unitario llama a `carrito_descuento` — esa función solo se ejercita desde `test_compra_con_descuento()`, que está en `test_integracion.c`, archivo que no entra en el cálculo de cobertura.
...
> R: Un test unitario verifica una función de forma aislada; un test de integración verifica que varias funciones trabajen bien combinadas entre sí. En este laboratorio, el test unitario `test_total_con_cantidad` fue el que detectó primero el bug de `carrito_total`.
...
> R: En C no hay verificación automática de límites de arrays: el lenguaje permite escribir en cualquier dirección de memoria calculada, incluso fuera del array, corrompiendo datos adyacentes (como otras variables en el stack), lo que puede provocar comportamiento indefinido, crashes difíciles de diagnosticar o vulnerabilidades de seguridad. Python y Java, en cambio, verifican los límites del array en tiempo de ejecución y lanzan una excepción controlada (`IndexError` o `ArrayIndexOutOfBoundsException`) en vez de permitir la escritura fuera de rango.
...
> R: Los tests verifican el comportamiento real del código frente a casos concretos, en vez de depender de que uno "lea bien" el código y note el error a simple vista. El bug de `carrito_total` era fácil de pasar por alto leyendo el código superficialmente, pero un test con cantidad=2 lo expuso de forma clara e inequívoca. Además, una vez escritos, los tests quedan para detectar regresiones si el código se vuelve a romper en el futuro.
...
> R: Porque con cantidad=1, multiplicar por la cantidad o no hacerlo da el mismo resultado (precio × 1 = precio). El bug de no multiplicar por la cantidad solo se manifiesta cuando la cantidad es distinta de 1, como en el segundo test (cantidad=2).
